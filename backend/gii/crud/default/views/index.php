...
if (($tableSchema = $generator->getTableSchema()) !== false) {
    ...
    foreach ($tableSchema->columns as $column) {
        if (substr($column->name, -3) == '_id') {
            $flag_select2 = true;
            $imported_models[] = str_replace(StringHelper::basename($generator->modelClass), Inflector::camelize(rtrim($column->name, '_id')), $generator->modelClass);
        }
        if ($column->type == 'date') {
...
<div class="<?= Inflector::camel2id(StringHelper::basename($generator->modelClass)) ?>-index box-- box-primary-- box-body--">
        ...
        foreach ($tableSchema->columns as $column) {
            ...
            if (substr($column->name, -3) == '_at') {
                ...
                if ($column->name == 'id') {
                    ...
                    echo "            '".lcfirst(Inflector::camelize(rtrim($column->name, '_id'))) . ".name:text:".Inflector::humanize(rtrim($column->name, '_id'))."',\n";
    ...
    if (($tableSchema = $generator->getTableSchema()) === false) {
        ...
        foreach ($tableSchema->columns as $column) {
            ...
            if ($column->name != 'created_at' && $column->name != 'updated_at' && $column->name != 'created_by' && $column->name != 'updated_by') {
                ...
                'value' => '" . lcfirst(Inflector::camelize(rtrim($column->name, '_id'))) . ".name',
                'filterType' => GridView::FILTER_SELECT2,
                'filter' => ArrayHelper::map(" . Inflector::camelize(rtrim($column->name, '_id')) . "::find()->orderBy('name')->asArray()->all(), 'id', 'name'), 
