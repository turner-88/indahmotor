...
$sum_in     = BalanceHistory::find()->andWhere(['<=', 'date', $to])->sum('`in`');
$sum_out    = BalanceHistory::find()->andWhere(['<=', 'date', $to])->sum('`out`');
$balance_after = $sum_in - $sum_out;
...
<div class="detail-view-container" style="padding:<?= isset($to_pdf) && $to_pdf ? '0px' : '20px' ?>">
    <div class="printable">
        ...
        <?php } else { ?>
            ...
            <div class="text-right"><b>SISA SALDO : Rp <?= Yii::$app->formatter->asDecimal($balance_after, 0) ?></b></div>
            <p></p>
            ...
            ?>
                <tr>
                    ...
                    <td class="text-right"><?= Yii::$app->formatter->asDecimal($balance_after, 0) ?></td>
                </tr>
            <?php 
                if ($model->transaction_type == 'i') $balance_after -= $model->in;
                if ($model->transaction_type == 'o') $balance_after += $model->out;
            } ?>
            </table>

            <div class="text-right"><b>SALDO AWAL : Rp <?= Yii::$app->formatter->asDecimal($balance_after, 0) ?></b></div>
